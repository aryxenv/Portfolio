...
function addItem() {
  ...
  if (
    !animeList.some(
      (anime) => anime.input.toLowerCase() === animeName.toLowerCase(),
    )
  ) {
    // animeList.push({
    //     input: animeName,
      ...
      .then((data) => {
        ...
        if (data.data && data.data.length > 0) {
          ...
          if (status == "Currently Airing") {
            ...
            const formattedDate =
              `${month.toString().padStart(2, "0")}/` + // Format month to 2 digits
            ...
            const hours = Math.floor(
              (differenceSeconds % (60 * 60 * 24)) / (60 * 60),
            );
            ...
            svgElement.setAttribute(
              "style",
              "margin-left:5px; width: 1em; height: 1em; vertical-align: middle; fill: currentcolor; --darkreader-inline-fill: currentcolor;",
            );
            ...
            if (title.includes("Unnamed Memory Season 2")) {
              ...
              const linkElement = document.createElement("a");
              linkElement.href =
                "https://hianime.to/watch/unnamed-memory-season-2-19440";
              linkElement.target = "_blank";
            ...
            // }
              ...
              fetch(proxyUrl + encodeURIComponent(linkToAnime))
                ...
                .then((data) => {
                  ...
                  if (linkElement) {
                    const linkToWatch =
                      "https://hianime.to" + linkElement.getAttribute("href"); // Construct the full URL
                    ...
                    // newItem.appendChild(removeButton);
                    console.error(
                      "Error: Could not find the 'a.film-poster-ahref' element in the HTML.",
                    );
                  }
                })
                .catch((error) => {
            ...
            removeButton.addEventListener("click", function () {
              ...
              const currentAnimeName = this.parentNode
                .querySelector(".name")
                .textContent.trim();
              ...
              const index = animeList.findIndex(
                (anime) =>
                  anime.title.toLowerCase() === currentAnimeName.toLowerCase(),
              );
          ...
          document.querySelector(".addAnime span").classList.add("rotateAdd");

          setTimeout(() => {
            //document.querySelector(".addAnime").innerHTML = "+";
            document.querySelector(".addAnime").classList.remove("unableToAdd");
            document
              .querySelector(".addAnime span")
              .classList.remove("rotateAdd");
            document.querySelector(".addAnime span").classList.add("unrotate");
          }, 2000);

          document.querySelector(".addAnime span").classList.remove("unrotate");
        }
      })
      .catch((error) => {
