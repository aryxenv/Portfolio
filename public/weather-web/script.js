...
document.querySelector(".linkedin").addEventListener("click", function () {
  window.open(
    "https://www.linkedin.com/in/aryan-shah-752364295/details/projects/?profileUrn=urn%3Ali%3Afsd_profile%3AACoAAEdr-S4BJSBfYJmGo7g8ZFQoH0BDic4IBew",
    "_blank",
  );
});
...
document.addEventListener("keydown", function (e) {
  if (
    document
      .querySelector(".islandTopCenter")
      .classList.contains("initialPhase")
  ) {
    if (e.key === "Enter") {
      searchP1(e);
    }
...
// fullscreen
document
  .querySelector(".islandFullscreen")
...
function animate(location) {
  ...
  document
    .querySelector(".islandBottomCenter")
    .classList.remove("initialPhaseBottom", "hidden");
...
function displayMainData(data, index = 0) {
  ...
  const wind_direction =
    data.forecast.forecastday[index].hour[hourLocation].wind_dir;
...
function displayHourlyData(data, index) {
  ...
  for (var hour = start; hour < end; hour++) {
    ...
    let time =
      String(new Date(hourly.time).getHours()).padStart(2, "0") + ":00";
...
async function fetchWeather(location) {
  try {
    ...
    const forecastResponse = await fetch(
      `https://api.weatherapi.com/v1/forecast.json?key=${weatherKey}&q=${location}&days=3&aqi=yes`,
    );
    ...
    const historyResponse = await fetch(
      `https://api.weatherapi.com/v1/history.json?key=${weatherKey}&q=${location}&dt=${pastDateStr}&end_dt=${endDateStr}`,
    );
    const historyData = await historyResponse.json();

    // Fetch images from Pexels using the country from the weather data
    const pexelResponse = await fetch(
      `https://api.pexels.com/v1/search?query=${forecastData.location.country}&per_page=5`,
      {
        headers: {
...
async function fetchIp() {
  try {
    const response = await fetch(
      `https://api.weatherapi.com/v1/ip.json?key=${weatherKey}&q=auto:ip`,
    );
