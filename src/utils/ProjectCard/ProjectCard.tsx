...
const ProjectCard : React.FC<ProjectCardProps> = ({id, title, description, websiteLink, githubLink, status}) => {
    ...
    const updateMinHeight = useCallback(() => {
        ...
        if (descriptionRef.current) totalHeight += descriptionRef.current.scrollHeight;
        if (projectCardLinksRef.current) totalHeight += projectCardLinksRef.current.scrollHeight;
    ...
    return (
        ...
        onClick={() => websiteLink !== '' ? window.open(websiteLink, '_blank') : window.open(githubLink, '_blank')} 
        ...
        key={id}>
            <div className='blurLayer'>
                ...
                {description.trim() !== '' && (
                    isSmallScreen ? (
                        ...
                        <p ref={descriptionRef} className={`project-description ${isHover ? '' : 'hidden'}`}>
                            {description}
                        </p>
                    )
                ...
                <div ref={projectCardLinksRef} className='project-card-links'>
                    {websiteLink.trim() !== '' && (
                        <div className='link-to-website' onClick={(e) => {e.stopPropagation(); window.open(websiteLink, '_blank')}}>
                            <i className='bx bx-world'></i>
                         </div>
                    )}
                    
                    {githubLink.trim() !== '' && (
                        <div className='link-to-github' onClick={(e) => {e.stopPropagation();  window.open(githubLink, '_blank')}}>
            ...
            <div className='project-card-status-container' >
                ...
                <div className={`project-card-status ${status !== 'none' ? `project-card-status-${status}` : ''}`}
                onMouseEnter={() => setIsHoverTooltip(true)}
                onMouseLeave={() => setIsHoverTooltip(false)}></div>
