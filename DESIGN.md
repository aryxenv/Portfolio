...
- **Status Live** (`green`), **Status Paused** (`yellow`), **Status Archived** (`red`): 10px dots on project cards and the pulsing 14px dot on the current role in the experience timeline. These are the only hues in the system besides the reactor, and they exist purely to encode state.
...
- **Items:** four section links plus Blog. Section links are plain in-page anchors while on `/`, and become router links to `/#section` from any other route.
...
- **Post page:** `h1` at `2.85rem` (`1.9rem` below 768px), a single `0.855rem` meta line, body at `0.95rem`, section headings at `1.425rem`.
- **Blocks:** post content is a typed union — paragraph, heading, list, quote, code. A quote is a full hairline panel in italic at 70%, not a left rule. Adding a block type means designing it once; there is deliberately no markdown escape hatch.
- **Code:** still Manrope. Code blocks and inline code use `white-space: pre` and tabular figures inside a hairline panel at `0.855rem` — a monospace face would be the second typeface this system refuses.
...
- **Don't** introduce a warm or tinted neutral anywhere. This system is `#000000` and `#FFFFFF` at varying alpha, and that austerity is the point.
